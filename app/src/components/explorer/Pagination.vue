...
const verifyRow = (): void => {
  ...
  const currentRowNumber = Math.ceil(props.cpage / lengths.value);
  if (props.cpage < props.tpages && currentRowNumber <= limit) {
    offset.value = (currentRowNumber - 1) * lengths.value;
    rowNumber.value = currentRowNumber;
  } else {
    goToLastRow();
  }
};
