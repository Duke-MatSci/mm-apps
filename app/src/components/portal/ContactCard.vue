<template>
  <div class="">
    <div class="gallery-item results_card" v-if="contact && !!Object.keys(contact).length">
      <md-card-header>
        <div class="results_card-title md-body-2" style="text-transform: capitalize">
          {{ contact.fullName }}
        </div>
        ...
        <div class="md-layout">
          ...
          <div v-else class="article_metadata u--color-secondary utility-line-height-sm">
            {{ contact.message }}
          </div>
        </div>
        <div v-if="show && hasAttachment" class="md-layout md-alignment-center-space-between">
          <p class="md-body-2">{{ reducedName }}.....</p>

          <md-button class="md-icon-button" @click="downloadImage(contact.attachments[0])">
...
const shortMessage = computed(() => {
  return reduceDescription(props.contact.message, 15);
});

const parseMessage = computed(() => {
  const parser = new DOMParser();
  const doc = parser.parseFromString(props.contact.response, 'text/html');
  return doc.getElementById('mainMessage')?.innerHTML ?? props.contact.response;
});
...
async function downloadImage(arg: string) {
...
function renderDialog(id: string) {
