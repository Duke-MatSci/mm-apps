<template>
  <div class="data-selector-wrapper">
    <div class="data-row u_margin-top-small">
      <div class="u--layout-flex u_searchimage_input u_centralize_items u--layout-flex-justify-sb">
      ...
      <md-table
        ...
        md-sort-order="asc"
      >
        <md-table-empty-state md-label="No data available"> </md-table-empty-state>
        <template #md-table-row="{ item }">
          <md-table-row :key="item.name">
            <md-table-cell md-label="Name" md-sort-by="name">
              <md-chip :style="{ 'background-color': item.color, color: 'white' }">
                {{ item.name.split('-').at(-1) }}
              </md-chip>
            </md-table-cell>
            <md-table-cell md-label="Select">
              <md-checkbox v-model="selectedValue" @change="onSelect" :value="item"></md-checkbox>
            </md-table-cell>
          </md-table-row>
        </template>
      </md-table>
...
defineOptions({ name: 'DataSelector' });
...
const query1Value = ref('C11');
const query2Value = ref('C12');
const selectedValueArr = ref<any[]>([]);
const jsonName = ref('');
...
const selectedValue = computed({
  get() {
    return fetchedNames.value.filter((item: any) =>
      activeData.value.map((data: any) => data.name).includes(item.name)
    );
  },
  set(val: any) {
    onSelect(val);
...
const disableInput = computed(() => {
  return (
    !dynamfit.value.fileUpload || !dynamfitData.value || !Object.keys(dynamfitData.value).length
  );
});
...
const onSelectInfo = (items: any[]) => {
  // Implementation depends on showDropDown which seems to be missing
};
...
const onSelect = (items: any[]) => {
  ...
  const unselected = fetchedNames.value.filter((item: any) => !selected.includes(item));

  selected.map((dataNameObj: any, index: number) => {
    let sourceItems = activeData.value;
    let destItems = dataLibrary.value;
    const checked = destItems.filter((item: any) => item.name === dataNameObj.name);
    destItems = destItems.filter((item: any) => item.name !== dataNameObj.name);
  ...
  unselected.map((dataNameObj: any, index: number) => {
    let sourceItems = dataLibrary.value;
    let destItems = activeData.value;
    const unchecked = destItems.filter((item: any) => item.name === dataNameObj.name);
    destItems = destItems.filter((item: any) => item.name !== dataNameObj.name);
...
const downloadFile = async () => {
  ...
  for (let i = 0; i < selectedValue.value.length; i++) {
    ...
    if (jsonData.value) {
      // Note: This ref access might need adjustment for Vue 3
      const elem = document.getElementById(jsonName.value);
      if (elem) {
        elem.click();
...
// Lifecycle
onMounted(() => {
  query1Value.value = query1.value ? query1.value : (route.query.pairwise_query1 as string);
  query2Value.value = query2.value ? query2.value : (route.query.pairwise_query2 as string);
