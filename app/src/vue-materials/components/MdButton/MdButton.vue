<template>
  <component
    :is="buttonComponent"
    :class="buttonClasses"
    :type="buttonType"
    :disabled="disabled || mdLoading"
    v-bind="buttonAttrs"
    v-on="buttonEvents"
    @click="handleClick"
    @touchstart="handleTouchStart"
    @touchmove="handleTouchMove"
    @mousedown="handleMouseDown"
  >
    <md-button-content
      :md-ripple="rippleEnabled"
      :disabled="disabled"
      :md-ripple-active="rippleActive"
      @update:md-ripple-active="rippleActive = $event"
    >
      <slot />
    </md-button-content>
  </component>
</template>

<script setup lang="ts">
import { computed, inject, ref } from 'vue';
import { useMaterial } from '../../material';
import MdButtonContent from './MdButtonContent.vue';

interface Props {
  href?: string;
  type?: string;
  disabled?: boolean;
  mdTheme?: string;
  mdType?: 'button' | 'submit' | 'reset';
  mdRaised?: boolean;
  mdOutlined?: boolean;
  mdFab?: boolean;
  mdMiniFab?: boolean;
  mdIcon?: boolean;
  mdSize?: 'small' | 'normal' | 'large';
  mdLoading?: boolean;
  mdRipple?: boolean;
  to?: string | object;
  exact?: boolean;
  exactActiveClass?: string;
  activeClass?: string;
  replace?: boolean;
  append?: boolean;
}

const props = withDefaults(defineProps<Props>(), {
  type: 'button',
  disabled: false,
  mdTheme: 'default',
  mdType: 'button',
  mdRaised: false,
  mdOutlined: false,
  mdFab: false,
  mdMiniFab: false,
  mdIcon: false,
  mdSize: 'normal',
  mdLoading: false,
  mdRipple: undefined, // Will be set from material instance if undefined
  exact: false,
  replace: false,
  append: false,
});

const emit = defineEmits<{
  click: [event: Event];
  touchstart: [event: Event];
  touchmove: [event: Event];
  mousedown: [event: Event];
}>();

// Get material instance
const material = useMaterial();

// Inject theme from parent component
const mdActiveTheme = inject(
  'mdActiveTheme',
  computed(() => `md-theme-${props.mdTheme}`)
);

// Reactive data
const rippleActive = ref(false);
const mdHasFocus = ref(false);

// Computed
const rippleEnabled = computed(() => {
  // Use material instance ripple setting if not explicitly set
  return props.mdRipple !== undefined ? props.mdRipple : material?.ripple ?? true;
});

const rippleWorks = computed(() => {
  return rippleEnabled.value && !props.disabled;
});

const isRouterLink = computed(() => {
  // In Vue 3, we need to check for router differently
  return !!props.to;
});

const buttonType = computed((): 'button' | 'submit' | 'reset' => {
  return props.mdType as 'button' | 'submit' | 'reset';
});

const buttonClasses = computed(() => {
  return {
    'md-button': true,
    [`md-theme-${props.mdTheme}`]: true,
    'md-ripple-off': !rippleEnabled.value,
    'md-focused': mdHasFocus.value,
    'md-flat':
      !props.mdRaised && !props.mdOutlined && !props.mdFab && !props.mdMiniFab && !props.mdIcon,
    'md-raised': props.mdRaised,
    'md-outlined': props.mdOutlined,
    'md-fab': props.mdFab,
    'md-mini-fab': props.mdMiniFab,
    'md-icon-button': props.mdIcon,
    [`md-size-${props.mdSize}`]: props.mdSize !== 'normal',
    'md-loading': props.mdLoading,
    'md-disabled': props.disabled || props.mdLoading,
  };
});

const buttonComponent = computed(() => {
  if (isRouterLink.value) {
    return 'router-link';
  }
  return 'button';
});

const buttonAttrs = computed(() => {
  if (isRouterLink.value) {
    return {
      to: props.to,
      exact: props.exact,
      exactActiveClass: props.exactActiveClass,
      activeClass: props.activeClass,
      replace: props.replace,
      append: props.append,
    };
  }
  return {
    type: buttonType.value,
  };
});

const buttonEvents = computed(() => {
  return {};
});

// Methods
const handleTouchStart = (event: Event) => {
  if (rippleWorks.value) {
    rippleActive.value = true;
  }
  emit('touchstart', event);
};

const handleTouchMove = (event: Event) => {
  if (rippleWorks.value) {
    rippleActive.value = true;
  }
  emit('touchmove', event);
};

const handleMouseDown = (event: Event) => {
  if (rippleWorks.value) {
    rippleActive.value = true;
  }
  emit('mousedown', event);
};

const handleClick = (event: Event) => {
  emit('click', event);
};

defineOptions({
  name: 'MdButton',
  components: {
    MdButtonContent,
  },
});
</script>

<style lang="scss">
@import '../MdAnimation/variables.scss';
@import '../MdElevation/mixins.scss';
@import '../MdElevation/variables.scss';
@import './theme.scss';

$md-button-min-width: 88px;
$md-button-height: 36px;
$md-button-radius: 2px;
$md-button-font-size: 14px;

$md-button-dense-height: 32px;

$md-button-icon-size: 40px;

$md-button-fab-size: 56px;
$md-button-fab-size-mini: $md-button-icon-size;

.md-button-clean {
  margin: 0;
  padding: 0;
  display: inline-block;
  position: relative;
  overflow: hidden;
  outline: none;
  background: transparent;
  border: 0;
  border-radius: 0;
  transition: $md-transition-default;
  font-family: inherit;
  line-height: normal;
  text-decoration: none;
  vertical-align: top;
  white-space: nowrap;
}

.md-button {
  @extend .md-button-clean;
  min-width: $md-button-min-width;
  height: $md-button-height;
  margin: 6px 8px;
  user-select: none;
  border-radius: $md-button-radius;
  font-size: $md-button-font-size;
  font-weight: 500;
  text-transform: uppercase;

  &:active {
    outline: none;
  }

  &[disabled] {
    pointer-events: none;
  }

  &:not([disabled]) {
    cursor: pointer;

    &:hover,
    &:active,
    &.md-focused {
      &:before {
        background-color: currentColor;
        opacity: 0.12;
      }
    }

    &.md-focused {
      &.md-primary,
      &.md-accent {
        &:before {
          opacity: 0.2;
        }
      }
    }

    &:active {
      &:before {
        opacity: 0.2;
      }
    }

    &.md-ripple-off:active:before {
      opacity: 0.26;
    }
  }

  &.md-plain.md-button.md-raised:not([disabled]) {
    color: rgba(#000, 0.87);
    background-color: #fff;

    .md-icon-font {
      color: rgba(#000, 0.87);
    }

    .md-icon-image {
      fill: rgba(#000, 0.87);
    }
  }

  &::-moz-focus-inner {
    padding: 0;
    border: 0;
  }

  &:before {
    position: absolute;
    top: 0;
    right: 0;
    bottom: 0;
    left: 0;
    z-index: 1;
    opacity: 0;
    transition: $md-transition-default;
    will-change: background-color, opacity;
    content: ' ';
  }

  &.md-dense {
    height: $md-button-dense-height;
    font-size: 13px;
  }

  &.md-raised:not([disabled]) {
    @include md-elevation(2);

    &:active {
      @include md-elevation(8);
    }

    &.md-ripple-off:active:before {
      opacity: 0.2;
    }
  }

  + .md-button {
    margin-left: 0;
  }

  .md-ripple {
    padding: 0 8px;
    display: flex;
    justify-content: center;
    align-items: center;
  }
}

.md-button-spaced .md-ripple {
  padding: 0 16px;
}

.md-icon-button,
.md-fab {
  border-radius: 50%;
  z-index: 10;

  &:before {
    border-radius: 50%;
  }

  .md-ripple {
    border-radius: 50%;
  }
}

.md-icon-button,
.md-fab.md-mini,
.md-fab.md-dense {
  .md-ripple-wave {
    top: 0 !important;
    right: 0 !important;
    bottom: 0 !important;
    left: 0 !important;
  }
}

.md-icon-button {
  width: $md-button-icon-size;
  min-width: $md-button-icon-size;
  height: $md-button-icon-size;
  margin: 0 6px;

  &.md-dense {
    width: $md-button-dense-height;
    min-width: $md-button-dense-height;
    height: $md-button-dense-height;
  }

  .md-ripple-enter-active {
    transition-duration: 1.2s;
  }
}

.md-fab {
  @include md-elevation(6);

  width: $md-button-fab-size;
  height: $md-button-fab-size;
  min-width: 0;
  overflow: hidden;

  &:active {
    @include md-elevation(12);
  }

  &.md-mini,
  &.md-dense {
    width: $md-button-fab-size-mini;
    height: $md-button-fab-size-mini;
  }

  &.md-fab-top-right,
  &.md-fab-top-left {
    position: absolute;
    top: 24px;
  }

  &.md-fab-bottom-right,
  &.md-fab-bottom-left {
    position: absolute;
    bottom: 24px;
  }

  &.md-fab-top-center,
  &.md-fab-bottom-center {
    position: absolute;
    left: 50%;
    transform: translateX(-50%);
  }

  &.md-fab-top-center {
    top: 24px;
  }

  &.md-fab-bottom-center {
    bottom: 24px;
  }

  &.md-fab-top-right,
  &.md-fab-bottom-right {
    right: 24px;
  }

  &.md-fab-top-left,
  &.md-fab-bottom-left {
    left: 24px;
  }

  &.md-fixed {
    position: fixed;
  }

  .md-ripple {
    padding: 0;
  }
}
</style>
