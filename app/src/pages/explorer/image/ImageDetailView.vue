      ...
      <div class="utility-content__result teams_partner">
        <div class="utility-space search_box_form u--layout-flex-justify-end">
          ...
          <md-button
            ...
            @click.prevent="handleShare(`${baseUrl}${route.fullPath}`)"
          >
...
const { reduceDescription, reduceAsset, hideAssetNavLeft } = useReduce();

// Reactive data
const baseUrl = ref(window.location.origin);
const shareToolTip = ref('Share Image');
const error = ref({ status: false, message: null });
const getSingleImages = ref({} as any);
...
const currentImage = ref({} as any);
const assetItems = ref([] as any);
const pushedAssetItem = ref([] as any);
const screen = ref(0);

// Apollo query
const { result: singleImageResult, loading } = useQuery(
  SINGLE_IMAGE_QUERY,
  computed(() => ({
...
watch(singleImageResult, (newValue) => {
  if (newValue) {
    getSingleImages.value = newValue.getSingleImages || {};
    setCurrentImage(newValue.getSingleImages);
  }
});
