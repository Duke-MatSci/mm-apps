...
const chart = ref({
  baseSpec: null as any,
  query: null as string | null,
  title: null as string | null,
  description: null as string | null,
});
const actionType = ref('Save Chart');
const submittedIdentifier = ref(undefined as any);
...
const onQuerySuccess = (resultsData: any) => {
...
const saveChart = async () => {
  ...
  try {
    const chartNanopub = await saveChart(chart.value);
    ...
    const resp = await store.dispatch('explorer/curation/cacheNewEntityResponse', {
    ...
    if (route.params.type === 'new') {
      return router.push(`/explorer/chart/editor/edit/${toChartId(resp.identifier)}`);
    }
  } catch (err: any) {
