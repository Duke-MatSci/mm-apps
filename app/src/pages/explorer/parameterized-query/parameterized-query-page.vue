...
import {
  loadSparqlTemplates as loadSparqlTemplatesFn,
  TextSegmentType,
  OptValueType,
  type Template,
...
const queryTemplates = ref<Record<string, Template>>({});
...
const templateIds = computed(() => {
  return Object.keys(queryTemplates.value);
});

const selectedTemplate = computed(() => {
  return queryTemplates.value[selTemplateId.value!] as any;
});
...
const loadSparqlTemplates = async () => {
  ...
  try {
    const templates = await loadSparqlTemplatesFn();
...
const populateSelections = () => {
  ...
  varSelections.value = Object.fromEntries(
    Object.entries(selectedTemplate.value.options).map(([varName, varOpts]: [string, any]) => [
      varName,
...
const buildQuery = () => {
  ...
  const activeReplacements = Object.fromEntries(
    Object.entries(selectedTemplate.value.replacements).map(([varName, varObj]: [string, any]) => {
...
const updatePageTitleArray = () => {
  if (queryTemplates.value && Object.keys(queryTemplates.value).length > 0) {
    pageTitle.value = Object.keys(queryTemplates.value).map((key) => {
      return (
        queryTemplates.value[key].display
          .match(/<b>(.*?)<\/b>/g)
          ?.map((val) => {
