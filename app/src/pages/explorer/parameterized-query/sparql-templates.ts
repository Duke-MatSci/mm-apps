...
export async function loadSparqlTemplates(): Promise<Template[]> {
  ...
  for (const templateRdf of templateRdfStrs) {
    const store = new N3.Store() as any;
    const parser = new N3.Parser();
    await parser.parse(templateRdf, (_: any, quad: any, _prefixes: any) => {
      if (quad) {
        store.addQuad(quad);
      }
    });
...
export type TextSegmentType = (typeof TextSegmentType)[keyof typeof TextSegmentType];
...
export type OptValueType = (typeof OptValueType)[keyof typeof OptValueType];

function literalValue(lNode: any): any {
  return fromRdf(literal(lNode.value, namedNode(lNode.datatypeString)));
}

function getSingletonObject(store: any, s: any, p: string, optional = false): any | null {
...
function getSingletonLiteral(store: N3.Store, s: any, p: string, optional = false): any {
  const object = getSingletonObject(store, s, p, optional);
  return object && literalValue(object);
}

function buildTemplate(store: N3.Store, tNode: any): Template {
...
function buildTemplateReplacements(store: any, tNode: any): Record<string, Replacement> {
...
function buildTemplateParams(store: any, tNode: any): Record<string, Record<string, OptValue>> {
  return Object.fromEntries(
    store
      .getObjects(tNode, `${SPIN}constraint`)
      .map((paramNode: any) => [
        getSingletonLiteral(store, paramNode, `${SP}varName`),
...
function buildParamOpts(store: any, paramNode: any): Record<string, OptValue> {
  return Object.fromEntries(
    store
      .getObjects(paramNode, `${SCHEMA}option`)
      .map((optNode: any) => [
        getSingletonLiteral(store, optNode, `${RDFS}label`),
        getOptValue(store, optNode),
        getSingletonLiteral(store, optNode, `${SCHEMA}position`),
      ])
      .sort((e1: any, e2: any) => (e1[2] > e2[2] ? 1 : -1))
  );
}

function getOptValue(store: N3.Store, optNode: any): OptValue {
  let value: OptValue = {
...
function parseDisplayText(displayText: string): DisplaySegment[] {
  return displayText.match(segmentRegex)!.map((token) => {
    let displaySegment: DisplaySegment;
