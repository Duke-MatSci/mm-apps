                ...
                <md-steppers class="form__stepper" v-model:md-active-step="active" md-linear>
                  <md-step id="first" md-label="Upload files" :md-error="invalid.first">
                    <div style="margin: 20px">
                      ...
                      <file-input @files-dropped="addDistr">
                      ...
                      <file-input @files-dropped="previewFile()">
...
const doi = ref('');
const orcidId = ref(null);
const searchKeywordOrg = ref('');
const showDropdown = ref(false);
const distrFiles = ref(distrFn.files);
const depiction: any = ref(null);
const dataset = ref({ ...DATASET } as any);

// For editing existing datasets
const oldDistributions: any = ref([]);
const toDelete: any = ref(null);
const oldDepiction: any = ref(null);
...
const secondPageFilled = computed(() => {
  return (
    !!dataset.value.title &&
    !!optionalChaining(() => dataset.value?.contactPoint?.['@id']) &&
...
const generatedUUID = computed(() => uuidv4());

// Watchers
watch(orcidData, (newValue, oldValue) => {
  if (newValue === 'invalid') {
    invalid.value.orcid = true;
  } else {
    invalid.value.orcid = false;
    dataset.value.contactPoint['@id'] = newValue?.['@id'];
    dataset.value.contactPoint.firstName = newValue?.['http://schema.org/givenName'][0]?.['@value'];
    dataset.value.contactPoint.lastName = newValue?.['http://schema.org/familyName'][0]?.['@value'];
    dataset.value.contactPoint.cpEmail =
      newValue?.['http://www.w3.org/2006/vcard/ns#email']?.[0]?.['@value'];
  }
});
...
const setSnackbar = (payload: any) => store.commit('setSnackbar', payload);
const clearSnackbar = () => store.commit('resetSnackbar');
...
const removeImage = () => {
  const wrapper = document.querySelector('#depictWrapper') as HTMLElement;
  const fileInput = document.querySelector('#file-depict-input') as HTMLInputElement;
  const img = document.querySelector('#depictImg') as HTMLImageElement;
