...
const distributions = ref([] as any);
const isSddArray = ref([]);
const whichSdd = ref(null);
const selectSdd = ref('');
...
const checkInvalidThird = () => {
  let isInvalid = false;
  const csvs = distributions.value.filter((x: any) => x.fileExtension === 'csv');
...
const createDatasetLd = () => {
  ...
  let namespaceString = `${fullDatasetUri.value}/`;
...
const createFileLd = (index: number) => {
  ...
  if (distribution.fileExtension === 'csv') {
    ...
    if (whichSdd.value === 'other' && searchSdd.value) {
      ...
      const file = distributions.value.find((file: any) =>
        file.downloadLink.includes(selectSdd.value)
      );
      jsonLd['http://purl.org/dc/terms/conformsTo'] = {
        '@id': replaceBaseUrl(
          file?.downloadLink,
...
watch(dataset, (newValues) => {
  ...
  if (newValues?.[datasetFields.depiction]) {
    const thumbnailUri = dataset.value[datasetFields.depiction][0]['@id'];
    store.dispatch('explorer/fetchDatasetThumbnail', thumbnailUri);
  }
  if (newValues?.[datasetFields.distribution]) {
    for (const index in dataset.value[datasetFields.distribution]) {
      const downloadLink = dataset.value[datasetFields.distribution][index]?.['@id'];
