      ...
      <div class="curate">
        ...
        <div v-else>
          ...
          <md-steppers md-vertical md-linear v-model:md-active-step="active" class="form__stepper">
            ...
            <md-step id="second" md-label="Select spreadsheet for upload" v-model:md-done="second">
        ...
        <div v-if="dialog.type == 'datasetId'">
          ...
          >
            ...
            <template v-slot:md-autocomplete-empty="{ term }">
              <p style="max-width: 90%">No dataset IDs matching "{{ term }}" were found.</p>
            </template>
          </md-autocomplete>
        </div>
...
import LoginReq from '@/components/LoginRequired.vue';
...
// Reactive data
const auth = ref(true);
const uploadInProgress = ref('' as string);
const uploadResponse = ref(null as any);
const selectedDataset = ref({
  label: '',
  id: null,
});
const tifFiles: any = ref([]);
const newTifs = ref(false);
const renameXlsx = ref(false);
const spreadsheetFiles = ref(spreadsheetFn.files);
const suppFiles = ref(suppFn.files);
const doi = ref(null);
...
// Apollo queries using @vue/apollo-composable
const { result: verifyUserResult } = useQuery(VERIFY_AUTH_QUERY, null, {
  fetchPolicy: 'cache-and-network',
});

const { result: getUserDatasetResult } = useQuery(USER_DATASET_IDS_QUERY, null, {
  fetchPolicy: 'cache-and-network',
});

// Computed properties for Apollo results
const verifyUser = computed(() => verifyUserResult.value?.verifyUser || null);
const getUserDataset = computed(
  () => getUserDatasetResult.value?.getUserDataset || { datasets: [] }
);
