            ...
            <div class="md-layout" v-show="xmlFiles.length">
              <md-list class="md-layout utility-transparentbg md-theme-default">
                <FilePreview
                  ...
                  @remove="(file: any) => removeXmlFile(file)"
                />
              </md-list>
            </div>
            <md-button
...
const xmlFiles = ref(xmlFilesFn.files);
const loading = ref(false);
const uploadInProgress = ref(null as string | null);
...
const removeXmlFile = xmlFilesFn.removeFile;
const modStatXml = xmlFilesFn.modifyStatus;

const submitXML = (files: any) => store.dispatch('explorer/curation/submitXmlFiles', files);
...
const filterXml = (files: FileList | null) => {
  if (!files) return [];
  ...
  for (let i = 0; i < newFiles.length; i++) {
    if (!regex.test(newFiles[i].name)) {
      // renameXlsx would be set here if needed
...
const submitXmlFiles = async () => {
  ...
  loading.value = true;
  await submitXML(xmlFiles.value);
  loading.value = false;
