    ...
    >
      ...
      <div class="u_margin-bottom-small">
        ...
        <p class="u_margin-top-small search_box_link utility-line-height-sm">
          ChemProps is a growing polymer name and filler name standardization database. Polymer
          names and filler names can have many alias, impeding the queries to collect all relevant
          data by a chemical name as the keyword. ChemProps is a useful resource to address this
          problem. It standardizes the chemical names that belong to the same database such that
          queries can retrieve all data that is related to a certain chemical name regardless of how
          they were originally reported. You can find more details in our paper
          <a
            class="u_color"
      ...
      <div>
        <p class="u_margin-top-small search_box_link utility-line-height-sm">
          This webapp is designed for one-time search. For batch jobs, please use the ChemProps API.
          A token is required to use the API. You must be logged in to request the ChemProps API
          token.
        </p>
      </div>
      <div class="u_width--max">
        <div class="md-card-header" v-if="tokenVisible">
          ...
          <div style="word-wrap: break-word" class="md-card-header utility-gridborder">
            <code> {{ token }} </code>
          </div>
        ...
        <div class="u_centralize_content md-card-header u_display-flex u_margin-bottom-med">
          <md-button class="md-primary md-raised u_centralize_other btn--primary" @click="showToken"
            >Request API Token</md-button
          >
        </div>
        <div class="utility-line-height-sm">
          <h3>Instructions</h3>
          <form @submit.prevent="search" id="submit-chemprops-form" class="u_margin-bottom-med">
            <ol>
              <li>Select the collection.</li>
              <div>
                <md-radio class="md-primary" v-model="pfRadios" value="pol">Polymer</md-radio>
                <md-radio class="md-primary" v-model="pfRadios" value="fil">Filler</md-radio>
              </div>
              <li v-if="pfRadios === 'pol'">
                Input the searching terms. For the quick search, you can search by either chemical
                name, abbreviation, trade name, or SMILES. For the advanced search, you must input a
                chemical name.
              </li>
              <li v-if="pfRadios === 'fil'">
                Input the searching terms. Note that you must input a chemical name.
              </li>
              <div>
                <label for="keywordSearch" v-if="pfRadios === 'pol'">
                  Quick Search (Filling this textbox will overwrite the advanced search)
                </label>
                ...
                <label for="fullSearch" v-if="pfRadios === 'pol'">Advanced Search</label>
                <md-field>
                  <md-input
                    id="fullSearch"
              ...
              <md-button type="submit" class="md-primary md-raised" form="submit-chemprops-form"
                >Search</md-button
              >
            </ol>
          </form>
        ...
        >
          ...
          <div class="md-layout-item md-size-100">
            ...
            <md-field>
              <md-input id="standardName" v-model="standardName" readonly></md-input>
            </md-field>
...
// Reactive data
const title = ref('ChemProps');
const loading = ref(false);
const dialog = ref<Dialog>({
  title: '',
  content: '',
});
const pfRadios = ref('pol');
...
const references = ref(['10.1186/s13321-021-00502-6', '10.1021/acs.jcim.7b00425']);
...
const search = async () => {
  resetOutput();
  const ChemicalName = isQuickSearch.value ? quickSearchKeyword.value : chemicalName.value;
  const Abbreviation = isQuickSearch.value ? quickSearchKeyword.value : abbreviation.value;
  const TradeName = isQuickSearch.value ? quickSearchKeyword.value : tradename.value;
  ...
  try {
    ...
    density.value = parseFloat(response?.data?.density);
...
const formulaUpdated = (formula: string) => {
...
const renderDialog = (title: string, content: string) => {
  dialog.value = {
    title,
    content,
  };
